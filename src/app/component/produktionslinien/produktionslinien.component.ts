...
//console.log(this.produktionslinienDaten)
      ...
      this.http.put(`http://-:3002/aktualisiereStatus/${this.produktionslinienDaten}`, {
  ...
  private ladeAktivesProjekt() {
    this.http.get<any>(`http://-:3002/aktives-projekt/${this.produktionslinienDaten}`).subscribe(projekt => {
      if (projekt) {
...
private sendRequest(daten: DataToSend) {
  this.http.post('http://-:3002/data', daten)
    .subscribe({
      next: (response: any) => {
...
private neueauftrag(daten: DataToSend) {
  this.http.post(`http://-:3002/neuerAuftragMitarbeiter/${this.produktionslinienDaten}`,daten)
    .subscribe({
      next: (response: any) => {
        this.handleErfolgreicheAntwort(response, daten);
        //this.aktualisiereSeite();
...
private resetInput() {
  ...
  private sendRequestToBackend(mitarbeiterName: string) {
    ...
    const url = `http://-:3002/checkMitarbeiter/${this.produktionslinienDaten}`;
  ...
  async checkAktiverAuftrag(daten: DataToSend): Promise<any> {
    //console.log(daten)
    try {
      const url = 'http://-:3002/check-aktiver-auftrag';
  ...
  private scanAuftrag(auftragsnummer: string) {
    this.http.get<any>(`http://-:3002/auftrag-details/${auftragsnummer}`)
      .subscribe({
        next: (response) => {
